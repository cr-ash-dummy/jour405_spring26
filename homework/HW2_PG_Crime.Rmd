...
The incidents are organized alphabetically. Some categories are broader than others; there are four categories regarding different types of robbery and only one for sex crimes. Others have specific modifiers, like AUTO STOLEN and AUTO STOLEN & RECOVERED, meaning both categories must be combined to find the total number of attempted car thefts. Which categories are most important is a highly subjective question and would depend on the audience and the general angle of the piece. 
...
```{r}
pg_crime <- pg_crime |> mutate(total_incidents = `Jan`+`Feb`+`Mar`+`Apr`+`May`+`Jun`+`Jul`+`Sep`+`Oct`+`Nov`+`Dec`)

pg_crime_averages <- pg_crime |> mutate(average_incidents = (`Jan`+`Feb`+`Mar`+`Apr`+`May`+`Jun`+`Jul`+`Sep`+`Oct`+`Nov`+`Dec`)/12)
```
...
Accidents, stolen cars and theft from cars have the highest averages of all categories, while "other" breaking and entering, homicide, and "other" robbery have the lowest. This demonstrates that cars are relatively unsafe, both in terms of driving and due to their vulnerability to theft. I'm confused what the "other" tag means for both breaking and entering and robbery; what kinds of places get robbed that don't fall under residential or commercial?
...
There is some value to the theory. Assault shooting saw a massive spike during July and August, rising by about 50% during both months. Car thefts were consistently higher by 15-20% across the summer, as were thefts from cars. Homicides spiked by an even higher margin, hitting 52% in June before dropping to 33% for both July and August. Some crimes show little correlation or even a drop, like other breaking and entering, which is in the negatives for all three months.
...
New statistics homicides spike during summer months, per the Prince George's County Police Department.

In new crime statistics released for the year of 2025, the average number of homicides per month was only 5. Incidents jumped to 8 in June and 7 in July and August, an increase of 52% and 33% respectively. This leap leads credence to the common theory that violent crime increases during the summer months. Other crimes saw increases during the summer as well, including car thefts.
