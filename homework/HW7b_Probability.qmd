...
```{r}
ggplot(data = wh_paid, aes(x = SALARY)) + 
  geom_histogram() +
...
The distribution is skewed positive, towards the right end of the chart. It is not at all normally distributed; salaries cluster in a few places between 50k and 100k, again at 120k and at 190k.
...
The data mostly follows the rule, although it falls slightly short. Data within 1 standard deviation and 2 standard deviations from the mean are about 4% less than the empirical rule. So distribution is mostly normal.
...
The normal approximation is slightly lower than the actual percentage, meaning the data isn't totally distributed normally. There is a slightly higher chance in the actual data that an employee makes over $150k, so more people are paid higher than they would be in a normally distributed chart of salaries.
...
Salaries are clustered slightly towards extremes. Fewer employees make between 80k and 120k than expected.
...
Many more employees make between 60k and 90k than expected, and more make above 150k than expected. That 60k-90k range seems to be the average salary for low-level staffers, while >150k is for especially important people. It would make sense that the White House is mostly populated by lackeys paid pittances (for DC at least) and powerful, high paid officials.
...
White House salaries favor extremes, leave mid-level earners behind

Salaries for White House staffers vary widely. In an expected normal distribution of salaries, the majority would make between 90 thousand to 120 thousand a year, following a "bell curve" model where most data -- or in this case, pay -- is concentrated in the middle. However, staffer salaries favor the high and low ends of the scale. Earners making between 60 to 90 thousand a year are overrepresented by 73%, while earners making 90 to 120 thousand lag at -30%. 

^I KNOW THIS IS CLUMSILY WORDED. I'M NOT SURE HOW TO EXPLAIN THIS IN A BETTER WAY. I would hope our readers understand the metaphor of the bell curve? But I should probably explain where those percentages come from. And then explain really where all the data lands and how salaries are clustered generally.
