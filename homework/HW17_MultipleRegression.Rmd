...
The variables with the strongest relationship to the dependent variable are the percentage of white population and the percentage of unemployed people. White people are the biggest positive indicator and unemployed people are the biggest negative indicator. A higher percentage of bachelors degrees doesn't seem to have much bearing on turnout. There are plenty of other variables that aren't correlated much with each other, like education and median age, or percentage of white population and income.
...
The p-value for the model overall is incredibly small, so it's statistically significant. Income, median age and percentage of white population all have statistically significant p-values. Education and population both have p-values well over the threshold, so they're not significant, whereas unemployed percentage is closer. The adjusted r-squared is about 80%, which is pretty high. I'd say this is a useful model.
...
The model with only variables that showed up as statistically significant in the initial model performed the best.
...
# Model 3
model3 <- lm(turnout ~ income + median_age + pct_unemployed + pct_white, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ income + population, data = md_turnout)
summary(model4)
...
I guess I would talk about how these factors might influence voting decision? More income = more disposable money and time = able to take time off to go vote. I'd obviously need more inquiry than that, maybe people with a higher income are actually motivated to vote because they feel a greater urgency, like they have something to lose if candidates they don't support are successful. But you see the logic.
